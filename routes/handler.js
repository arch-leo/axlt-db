...
function _connectDB(conf, callback) {
    MongoClient.connect(conf.dbUrl,{ useNewUrlParser: true }, function (err, db) {
        if (err) {
            console.log(err);
        }
        callback(err, db);
...
var handler = {
    insertOne: function (conf, colName, doc, options, callback) {
        _connectDB(conf, function (err, db) {
            ...
            dbo.collection(colName).findOneAndUpdate({ count: colName }, inc, options, function (error, result) {
                switch (doc.belong) {
                    case '1':
            ...
            dbo.collection(colName).findOne(query, opts, function (err, doc) {
                ...
                if (doc) {
                    if (doc.times == 4 || doc.times == 9 || doc.times == 14) {
                        dbo.collection(colName).findOneAndUpdate({ name: query.name }, { $inc: { times: 1, award: 1 } }, options, function (error, result) {
                        ...
                        dbo.collection(colName).findOneAndUpdate({ name: query.name }, { $inc: { times: 1 } }, options, function (error, result) {
        ...
        _connectDB(conf, function (err, db) {
            ...
            dbo.collection(colName).findOne(query, opts, function (err, doc) {
                ...
                if (doc.award == 0) {
                    ...
                    dbo.collection(colName).findOneAndUpdate({ name: query.name }, { $inc: { award: -1 } }, options, function (error, result) {
    ...
    findMainUpdate: function (conf, colName, query, opts, options, callback) {
        _connectDB(conf, function (err, db) {
            ...
            dbo.collection(colName).findOneAndUpdate(words, { $set: update }, options, function (error, result) {
                callback(err, result);
                db.close();
            });
        });
    ...
    findItemClear: function (conf, colName, query, opts, options, callback) {
        _connectDB(conf, function (err, db) {
            ...
            dbo.collection(colName).updateMany({ type: 'vote' }, { $set: { times: 0, award: 0 } }, options, function (error, result) {
                callback(err, result);
                db.close();
            });
        });
    ...
    incTimer: function (conf, colName, query, opts, options, callback) {
        _connectDB(conf, function (err, db) {
            ...
            dbo.collection(colName).updateMany({ type: 'items' }, { $set: { times: 0, award: 0 } }, options, function (error, result) {
                callback(err, result);
                db.close();
            });
        });
